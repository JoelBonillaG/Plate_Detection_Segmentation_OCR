...
def evaluate_on_test(q):
    ...
    test_seq = CharMaskSequence(
        test_pairs, height=h, width=w, batch_size=BATCH_SIZE,
        sep_ratio=SEP_RATIO, shrink_y=SHRINK_Y,
        border_weight=BORDER_WEIGHT, shuffle=False, augment=False,
    )
    model.compile(
...
def main():
    ...
    q = dict(min_chars=MIN_CHARS, max_adj_ratio=MAX_ADJ_RATIO,
             max_row_spread=MAX_ROW_SPREAD, seed=SEED)
    ...
    print(f"  Input: {WIDTH}x{HEIGHT} = {pixels_per_image:,} px/imagen")
    print(f"  Bridge: {WIDTH//8}x{HEIGHT//8} = {bridge_positions} posiciones "
    ...
    print(f"Tamano de entrada: {WIDTH}x{HEIGHT}x1")
    print(f"sep_ratio={SEP_RATIO}  shrink_y={SHRINK_Y}  border_weight={BORDER_WEIGHT}")
    print(f"persp_prob={PERSP_PROB}  persp_warp={PERSP_WARP}")
    ...
    train_seq = CharMaskSequence(
        ...
        shuffle=True,
        augment=True,
        persp_prob=PERSP_PROB,
        persp_warp=PERSP_WARP,
    )
